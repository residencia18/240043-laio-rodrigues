...
public class FaturaDAO {
	public static boolean adicionarFatura(Fatura fatura, int idImovel) {
        try (Connection con = DAO.conectar();
            ...
            preparedStatement.setDate(1, (Date) fatura.getData().getTime());
            preparedStatement.setDouble(2, fatura.getValor());
            preparedStatement.setDouble(3, fatura.getLeituraAnterior());
            preparedStatement.setDouble(4, fatura.getLeituraAtual());
    ...
    private static Fatura criarFatura(ResultSet resultSet) throws SQLException {
        ...
        Double ultimaLeitura = resultSet.getDouble("ULTIMA_LEITURA");
        Double leituraAtual = resultSet.getDouble("LEITURA_ATUAL");
        boolean quitado = resultSet.getBoolean("QUITADO");

        return new Fatura(id, data, id, id, id, quitado, null)
    }
