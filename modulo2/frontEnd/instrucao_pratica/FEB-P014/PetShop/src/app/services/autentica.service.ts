...
export class AutenticaService {
  ...
  signupUser(email: string, password: string) {
    return this.http
      .post<AuthData>(
        'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAYdC8sO5Fe3OYdn767vJUIpsuxNBnVo1w',
        {
          email: email,
          password: password,
  ...
  loginUser(email: string, password: string) {
    return this.http
      .post<AuthData>(
        'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAYdC8sO5Fe3OYdn767vJUIpsuxNBnVo1w',
        {
          email: email,
          password: password,
          returnSecureToken: true,
